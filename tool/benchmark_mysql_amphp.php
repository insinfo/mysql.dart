...
function env_int(string $key, int $fallback): int
{
    ...
    return $parsed === false ? $fallback : $value;
}
...
function make_config(string $host, int $port, string $user, string $password, string $database): MysqlConfig
{
    return MysqlConfig::fromAuthority($host . ':' . $port, $user, $password, $database);
}

function connect_amp(MysqlConfig $config): SocketMysqlConnection
{
    return SocketMysqlConnection::connect(socketConnector(), $config);
}
...
function row_int_value($result): int
{
    $row = row_values($result->fetchRow());
    return (int)$row[0];
}
...
function ensure_benchmark_rows(SocketMysqlConnection $connection, int $targetRows, string $tableName): void
{
    $connection->query("
    ...
    $existingRows = row_int_value($connection->query("SELECT COUNT(*) FROM $tableName"));
...
function benchmark_result_set(
    SocketMysqlConnection $connection,
    string $tableName,
    int $size,
...
$benchTable = env_or_default('BENCH_TABLE', 'bench_rows_amphp');
...
if ($secure) {
    fwrite(STDERR, "amphp/mysql benchmark currently runs plain TCP only; set MYSQL_SECURE=false.\n");
    exit(64);
}

$config = make_config($host, $port, $user, $password, $database);
$connection = connect_amp($config);
$server = $connection->query('SELECT VERSION() AS version, @@version_comment AS comment, @@port AS port')->fetchRow();
$connection->close();
...
$textChecksum = 0;
for ($i = 0; $i < $warmupIterations; $i++) {
    $textChecksum += row_int_value($connection->query('SELECT 1'));
}
...
for ($i = 0; $i < $warmupIterations; $i++) {
    $autoPreparedChecksum += row_int_value($connection->execute('SELECT ? + ?', [40, 2]));
}
...
$preparedChecksum = 0;
for ($i = 0; $i < $warmupIterations; $i++) {
    $preparedChecksum += row_int_value($statement->execute([40, 2]));
}
