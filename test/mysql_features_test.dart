...
void main() {
  ...
  test('Transactional: Commit da transação', () async {
    await connection.execute("DROP TABLE IF EXISTS mysql_features_temp_test");
    await connection.execute(
        "CREATE TABLE mysql_features_temp_test (id INT AUTO_INCREMENT PRIMARY KEY, value INT)");
    await connection.execute(
        "INSERT INTO mysql_features_temp_test (value) VALUES (10), (20)");

    final updateResult = await connection.transactional((conn) async {
      final res = await conn.execute(
        "UPDATE mysql_features_temp_test SET value = :value",
        {"value": 500},
      );
    ...
    final result =
        await connection.execute("SELECT value FROM mysql_features_temp_test");
  ...
  test('Transactional: Rollback da transação em caso de erro', () async {
    await connection
        .execute("DROP TABLE IF EXISTS mysql_features_temp_test_rollback");
    await connection.execute(
        "CREATE TABLE mysql_features_temp_test_rollback (id INT AUTO_INCREMENT PRIMARY KEY, value INT) ENGINE=InnoDB;");
    await connection.execute(
        "INSERT INTO mysql_features_temp_test_rollback (value) VALUES (10), (20)");

    try {
      await connection.transactional((conn) async {
        await conn.execute(
          "UPDATE mysql_features_temp_test_rollback SET value = :value",
          {"value": 200},
        );
    ...
    final result = await connection
        .execute("SELECT value FROM mysql_features_temp_test_rollback");
  ...
  test('Prepare: Cria, executa e dealloca prepared statement', () async {
    ...
    await connection.execute(
        "CREATE TABLE mysql_features_temp_test (id INT AUTO_INCREMENT PRIMARY KEY, value INT)");
    await connection.execute(
        "INSERT INTO mysql_features_temp_test (value) VALUES (1), (2)");

    final stmt = await connection
        .prepare("UPDATE mysql_features_temp_test SET value = ?");
